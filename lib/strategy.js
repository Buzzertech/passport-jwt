 ...
 *          custom: If true, verify if the signature of the token is stored in redis db
 *          redisOptions: Provided, will run redis instance
...
function JwtStrategy(options, verify) {
    ...
    this._redisOptions = options.redisOptions
...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    this._secretOrKeyProvider(req, token, function(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            ...
            }, self._custom, self._redisOptions);
        }
    });
};
